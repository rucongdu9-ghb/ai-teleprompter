...
export async function POST(req: NextRequest) {
  ...
  const { action, content, product, points, audience, lang = "zh", phonetics = false, duration = "medium", category = "" } = body;

  const DURATION_MAP: Record<string, { label: string; wordCount: string; maxTokens: number }> = {
    short:  { label: "短脚本（3-5分钟直播 / 短视频）", wordCount: "约500-700字", maxTokens: 1200 },
    medium: { label: "中等直播（10-15分钟）",          wordCount: "约1500-2000字", maxTokens: 3000 },
    long:   { label: "长直播（20-30分钟）",            wordCount: "约3000-4500字，分8-10个段落", maxTokens: 5500 },
  };
  const dur = DURATION_MAP[duration] ?? DURATION_MAP.medium;
...
اكتب النص مباشرة بدون شرح.`;
      ...
      const categoryLine = category ? `商品类目：${category}\n` : "";
      prompt = `你是一位专业的直播带货文案专家。请为以下商品生成一套完整的直播脚本。

${categoryLine}商品：${product}
卖点：${points || "未提供"}
目标人群：${audience || "25-40岁女性"}
脚本长度：${dur.label}，${dur.wordCount}
...
格式要清晰，方便直接套用。`;
  ...
  try {
    const response = await fetch("https://api.deepseek.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        model: "deepseek-chat",
        max_tokens: action === "generate" ? dur.maxTokens : 3000,
